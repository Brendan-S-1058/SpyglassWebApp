...
def calcCopr(teamsInData, matchsInData, alliancesByMatch):
    ...
    keyList = []
    for i in range (len(teamsInData)):
        sortingDict[fins[0][i][0]+teamsInData[i]/100000] = teamsInData[i]
        keyList.append(fins[0][i][0]+teamsInData[i]/100000)

    keyList.sort(reverse=True)

    finalList = []
    for i in range (len(keyList)):
        finalList.append([(sortingDict[keyList[i]]), str(keyList[i]-sortingDict[keyList[i]]/100000)])
...
def Order (autolists, telelists, teamDir):
    ...
    for i in autolists:
        for i2 in telelists:
            if i[0] == i2[0]:
                labels.append([float(i[1])*100//1/100, float(i2[1])*100//1/100, int(i[0])])
